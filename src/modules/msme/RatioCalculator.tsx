            ...
            <CardHeader className="bg-gradient-to-r from-emerald-500/10 via-background to-background border-b border-border/10 p-4 md:px-6">
                <div className="flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <CardTitle className="text-xl font-black">Credit Ratios</CardTitle>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 overflow-hidden">
                        ...
                        <div className="lg:col-span-7 p-4 md:p-6 space-y-4 border-r border-border/50">
                            {activeTab === 'dscr' && (
                                <div className="grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-left-2 duration-300">
                                    <div className="space-y-2"><Label className="result-label text-primary">PAT (Profit After Tax)</Label><Input value={pat} onChange={e => setPat(e.target.value)} type="number" className="h-10 bg-accent/30 font-bold px-3 text-lg" /></div>
                                    <div className="space-y-2"><Label className="result-label text-primary">Depreciation</Label><Input value={dep} onChange={e => setDep(e.target.value)} type="number" className="h-10 bg-accent/30 font-bold px-3 text-lg" /></div>
                                    <div className="space-y-2"><Label className="result-label text-primary">Interest Expense</Label><Input value={interest} onChange={e => setInterest(e.target.value)} type="number" className="h-10 bg-accent/30 font-bold px-3 text-lg" /></div>
                                    <div className="space-y-2"><Label className="result-label text-primary">Total Obligation</Label><Input value={obligation} onChange={e => setObligation(e.target.value)} type="number" className="h-10 bg-accent/30 font-bold px-3 text-lg" /></div>
                                </div>
                            )}

                            {activeTab === 'liquidity' && (
                                <div className="grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-left-2 duration-300">
                                    <div className="space-y-2"><Label className="result-label text-primary">Current Assets</Label><Input value={ca} onChange={e => setCa(e.target.value)} type="number" className="h-10 bg-accent/30 font-bold px-3 text-lg" /></div>
                                    <div className="space-y-2"><Label className="result-label text-primary">Current Liab.</Label><Input value={cl} onChange={e => setCl(e.target.value)} type="number" className="h-10 bg-accent/30 font-bold px-3 text-lg" /></div>
                                    <div className="space-y-2 col-span-2"><Label className="result-label text-primary">Inventory (for Quick Ratio)</Label><Input value={inventory} onChange={e => setInventory(e.target.value)} type="number" className="h-10 bg-accent/30 font-bold px-3 text-lg" /></div>
                                </div>
                            )}

                            {activeTab === 'leverage' && (
                                <div className="grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-left-2 duration-300">
                                    <div className="space-y-2"><Label className="result-label text-primary">Total Outside Liab. (TOL)</Label><Input value={tol} onChange={e => setTol(e.target.value)} type="number" className="h-10 bg-accent/30 font-bold px-3 text-lg" /></div>
                                    <div className="space-y-2"><Label className="result-label text-primary">Tangible Net Worth (TNW)</Label><Input value={tnw} onChange={e => setTnw(e.target.value)} type="number" className="h-10 bg-accent/30 font-bold px-3 text-lg" /></div>
                                </div>
                            )}

                            {activeTab === 'bep' && (
                                <div className="grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-left-2 duration-300">
                                    <div className="space-y-2"><Label className="result-label text-primary">Fixed Costs</Label><Input value={fixedCost} onChange={e => setFixedCost(e.target.value)} type="number" className="h-10 bg-accent/30 font-bold px-3 text-lg" /></div>
                                    <div className="space-y-2"><Label className="result-label text-primary">Variable Costs</Label><Input value={varCost} onChange={e => setVarCost(e.target.value)} type="number" className="h-10 bg-accent/30 font-bold px-3 text-lg" /></div>
                                    <div className="space-y-2 col-span-2"><Label className="result-label text-primary">Total Projected Sales</Label><Input value={sales} onChange={e => setSales(e.target.value)} type="number" className="h-10 bg-accent/30 font-bold px-3 text-lg" /></div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="lg:col-span-5 p-4 md:p-6 bg-muted/30 flex flex-col justify-center space-y-4">
                            {activeTab === 'dscr' && (
                                <div className="space-y-4">
                                    <div className="space-y-1">
                                        <span className="result-label">DSCR Ratio</span>
                                        <div className={cn("hero-result-value h-12 flex items-center", dscr >= 1.25 ? "text-emerald-500" : "text-amber-500")}>
                                            {dscr.toFixed(2)}
                                        </div>
                                    </div>
                                    <div className="stat-card p-4 rounded-xl border border-border/50 bg-background shadow-sm flex items-center justify-between">
                                        ...
                                        <span className={cn("text-sm font-black uppercase tracking-widest px-3 py-1 rounded-lg shadow-inner",
                                            dscr < 1.0 ? "bg-red-500/10 text-red-600 border border-red-500/20" :
                                                dscr < 1.25 ? "bg-amber-500/10 text-amber-600 border border-amber-500/20" :
                            ...
                            {activeTab === 'liquidity' && (
                                <div className="space-y-4">
                                    <div className="space-y-1">
                                        ...
                                        <div className={cn("hero-result-value h-12 flex items-center", currentRatio >= 1.33 ? "text-emerald-500" : "text-amber-500")}>
                            ...
                            {activeTab === 'leverage' && (
                                <div className="space-y-4">
                                    <div className="space-y-1">
                                        <span className="result-label">TOL/TNW</span>
                                        <div className={cn("hero-result-value h-12 flex items-center", leverage <= 3 ? "text-emerald-500" : "text-red-500")}>
                                            {leverage.toFixed(2)}
                                        </div>
                                    </div>
                                    <div className="stat-card p-3 rounded-xl border-none flex items-center justify-between">
                                        <span className="result-label">Leverage</span>
                                        <span className={cn("text-[10px] font-black uppercase", leverage <= 3 ? "text-emerald-500" : "text-red-500")}>
                            ...
                            {activeTab === 'bep' && (
                                <div className="space-y-4">
                                    <div className="space-y-1">
                                        ...
                                        <div className="hero-result-value h-12 flex items-center text-emerald-500">
                                            {bep.toFixed(1)}%
                                        </div>
